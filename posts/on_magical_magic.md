...
The world has a mind of its own, and so too does magic. One rolls the dice when casting a spell [`2d6 or 3d6 for circumstance or legendary skill`]. A mage may tip the scales, so to speak, but at a cost. Balance will be restored one way or another. A mage feeling a spell failing may accept a `Burden` [represented by a `tarot card`] and have it succeed instead. How the Burden is satisfied is anyone’s guess, but rest assured, it will call on the mage in time. Particularly poor castings can invoke failure _and_ a Burden [rolling snake eyes]. A mage can endure only three before being lost in the world.
